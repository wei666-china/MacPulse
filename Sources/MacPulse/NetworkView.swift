...
struct NetworkView: View {
    ...
    private var linkCard: some View {
        LiquidCard {
            VStack(alignment: .leading, spacing: 11) {
                HStack {
                    SectionHeader(title: String(localized: "当前连接"), subtitle: link?.summary)
                    Spacer(minLength: 0)
                    StatusPill(
                        text: model.networkPath.isSatisfied ? String(localized: "已连接") : String(localized: "离线"),
                ...
                ValueRow(
                    title: String(localized: "当前发射速率"),
                    value: link?.linkRateMbps.map { String(format: "%.0f Mbps", $0) } ?? String(localized: "不可用"),
                    symbol: "gauge.with.dots.needle.67percent"
                )
                if link?.linkRateMbps != nil {
                    Text(String(localized: "瞬时链路速率,空闲时会自动降速省电;实测吞吐高于它属正常。"))
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
                ValueRow(
                    title: String(localized: "实时下行"),
                    value: liveRateText(model.current.deep.networkInBytesPerSecond),
                    symbol: "arrow.down.circle",
                    tint: .green
                )
                ValueRow(
                    title: String(localized: "实时上行"),
    ...
    private func liveRateText(_ bytesPerSecond: Double?) -> String {
        guard let bytesPerSecond, bytesPerSecond.isFinite, bytesPerSecond >= 0 else { return String(localized: "不可用") }
    ...
    private var interfaceText: String {
        ...
        default: return String(localized: "不可用")
        }
    }
    ...
    private var badges: [(text: String, symbol: String, tint: Color)] {
        ...
        if model.networkPath.isExpensive {
            result.append((String(localized: "按流量计费"), "dollarsign.circle", MacPulseTheme.warm))
        }
        if model.networkPath.isConstrained {
            result.append((String(localized: "低数据模式"), "tortoise", MacPulseTheme.warm))
        }
    ...
    private var consentCard: some View {
        LiquidCard {
            VStack(alignment: .leading, spacing: 12) {
                SectionHeader(title: String(localized: "网络测速"), subtitle: String(localized: "需要你先同意"))

                Text(String(localized: "开启后，MacPulse 会连接 Cloudflare 的公开测速节点来实测你的网速。这是这个 App 唯一会发出的网络请求。"))
                    .font(.callout)
                    .fixedSize(horizontal: false, vertical: true)

                VStack(alignment: .leading, spacing: 7) {
                    ConsentPoint(symbol: "paperplane", text: String(localized: "只发送无意义的填充字节用于计时，不发送设备信息、进程名、电池数据或任何标识符。"))
                    ConsentPoint(symbol: "eye", text: String(localized: "对方能看到的只有你的公网 IP 和由 IP 推断的大致地区——任何网络请求都无法避免这一点。"))
                    ConsentPoint(symbol: "internaldrive", text: String(localized: "结果只写入本机，不含 IP、Wi-Fi 名称或精确位置。"))
                    ConsentPoint(symbol: "arrow.up.arrow.down", text: String(localized: "完整测速每次约 65 MB；轻量检测约 35 KB。强度和自动测速都能在设置里改。"))
                }

                HStack(spacing: 9) {
                    Button(String(localized: "开启网络测速")) {
                        consentRaw = NetworkConsent.granted.rawValue
                    }
                    .buttonStyle(.borderedProminent)

                    Button(String(localized: "保持完全离线")) {
            ...
            VStack(alignment: .leading, spacing: 11) {
                HStack {
                    ...
                    if model.isNetworkTestRunning {
                        ...
                        Button(String(localized: "重新测速")) { model.requestManualNetworkTest() }
                            .buttonStyle(.bordered)
                            .controlSize(.small)
                    }
                }
                ...
                if let skip = model.networkSkipReason, model.networkPhase == nil {
                    Text(String(format: String(localized: "本次未测:%@"), String(describing: skip.title)))
                        .font(.caption)
                ...
                if let result = model.networkResult {
                    if result.connectivity != .online {
                        ...
                        Text(String(format: String(localized: "测速未完成:%@"), String(describing: result.connectivity.title)))
                            .font(.callout.weight(.semibold))
                        Text(String(localized: "本地链路正常不代表出网正常,常见于 DNS 或门户认证故障。"))
                            .font(.caption2)
                            .foregroundStyle(.tertiary)
                    } else {
                        resultGrid(result)
                    ...
                    Text(String(localized: "打开面板时会自动测速。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
    ...
    private func resultGrid(_ result: NetworkTestResult) -> some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
            ...
            MetricCard(
                title: String(localized: "延迟"),
                value: result.latency.map { String(format: "%.0f ms", $0.p50Milliseconds) } ?? String(localized: "不可用"),
                detail: result.latency.map { String(format: String(localized: "抖动 %.0f ms"), $0.jitterMilliseconds) } ?? "—",
                symbol: "timer",
                tint: MacPulseTheme.violet
            )
            MetricCard(
                title: String(localized: "负载延迟"),
                value: result.bufferbloatMilliseconds.map { String(format: "+%.0f ms", $0) } ?? String(localized: "不可用"),
    ...
    private func throughputText(_ estimate: ThroughputEstimate?) -> String {
        guard let estimate else { return String(localized: "不可用") }
    ...
    private func throughputDetail(_ estimate: ThroughputEstimate?) -> String {
        ...
        guard estimate.isTrustworthy else { return String(localized: "测量不稳定") }
        return String(format: String(localized: "±%.0f%% · %d 连接"), estimate.relativeSpread * 50, estimate.streams)
    }
    ...
    private func footnoteText(_ result: NetworkTestResult) -> String {
        ...
        if let samples = result.download?.samples, samples > 0 {
            parts.append(String(format: String(localized: "样本 %@ 次"), String(describing: samples)))
        }
        let totalBytes = result.bytesDownloaded + result.bytesUploaded
        if totalBytes > 0 {
            parts.append(String(format: String(localized: "用了 %@"), String(describing: MetricFormat.bytes(UInt64(totalBytes)))))
        }
        parts.append(String(localized: "测的是到最近的 Cloudflare 节点"))
    ...
    private func footnote(_ result: NetworkTestResult) -> some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            if result.completeness == .partial {
                Text(String(localized: "这次测量被中断，结果按已完成的部分给出，误差区间更宽。"))
                    .font(.caption2)
                    .foregroundStyle(MacPulseTheme.warm)
            }
        }
    ...
    private func detail(_ result: NetworkTestResult) -> some View {
        DisclosureGroup(String(localized: "详情")) {
            VStack(spacing: 7) {
                ValueRow(title: String(localized: "空闲延迟 p95"), value: result.latency.map { String(format: "%.0f ms", $0.p95Milliseconds) } ?? String(localized: "不可用"), symbol: "timer")
                ValueRow(title: String(localized: "服务端测得 RTT"), value: result.latency?.serverMinRttMilliseconds.map { String(format: "%.1f ms", $0) } ?? String(localized: "不可用"), symbol: "server.rack")
                ValueRow(title: String(localized: "握手失败"), value: result.latency.map { "\($0.failures)/\($0.attempts)" } ?? String(localized: "不可用"), symbol: "exclamationmark.triangle")
                if let utilisation = result.linkUtilisation {
                    ValueRow(title: String(localized: "链路利用率"), value: String(format: "%.0f%%", utilisation * 100), symbol: "percent")
                }
                ValueRow(title: String(localized: "IPv6 可达"), value: result.ipv6Reachable.map { $0 ? String(localized: "是") : String(localized: "否") } ?? String(localized: "不可用"), symbol: "6.circle")
                ValueRow(title: String(localized: "连通性"), value: result.connectivity.title, symbol: "checkmark.seal")

                Text(String(localized: "丢包率：未测量（需要 ICMP 探测）。TCP 的重传会把丢包表现成延迟升高，从 TCP 数据里印一个「丢包 0%」是编造。"))
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
                    .frame(maxWidth: .infinity, alignment: .leading)
                ...
                if let rate = result.link?.linkRateMbps, result.linkUtilisation != nil {
                    Text(String(
                        format: String(localized: "链路协商 %@ Mbps 与实测的差距属正常：Wi-Fi 是半双工，协商速率是理论上限。"),
    ...
    var trendCard: some View {
        LiquidCard {
            VStack(alignment: .leading, spacing: 10) {
                ...
                SectionHeader(title: String(localized: "测速趋势"), subtitle: String(format: String(localized: "%@ 次记录"), String(describing: trendPoints.count)))

                Picker(String(localized: "指标"), selection: $trendMetric) {
                    ForEach(NetworkTrendMetric.allCases) { Text(L($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented)
                .labelsHidden()

                Picker(String(localized: "范围"), selection: $trendRange) {
                    ForEach(NetworkTrendRange.allCases) { Text(L($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented)
                .labelsHidden()
                ...
                if points.isEmpty {
                    Text(String(localized: "这个范围内还没有可用记录。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    ...
                    Chart(points, id: \.id) { point in
                        LineMark(
                            x: .value(String(localized: "时间"), point.date),
                            y: .value(trendMetric.rawValue, point.value),
                            series: .value(String(localized: "网络"), point.series)
                        )
                        .foregroundStyle(by: .value(String(localized: "网络"), point.series))
                    ...
                    Text(String(localized: "每个点是一次实测，不是连续采样——两点之间没有数据。"))
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                }
            }
    ...
    var trendPoints: [TrendPoint] {
        ...
        return model.networkHistory.compactMap { record -> TrendPoint? in
            ...
            let series = record.networkLabel
                ?? record.networkKeyHash.map { String(format: String(localized: "网络 %@"), String(describing: $0.prefix(4))) }
                ?? (record.interfaceName ?? String(localized: "未知网络"))
